...
abstract class AbstractLexer {
	...
	private final char[] readBuffer = new char[READ_BUFFER_SIZE + 1]; // + 1 for
	protected int c;
	...
	private int readBuffered() throws IOException {
		...
		if (readBufferPos >= readBufferSize) {
			...
			// Manually specify range, since buffer size has one extra space for
			// low-surrogate characters if needed
			...
			// If we read a high-surrogate but haven't read a low-surrogate,
			// read low-surrogate as well
