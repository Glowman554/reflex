...
/**
 * Instances of the {@link Json} class define how JSON data is parsed and
 * formatted. It defines whether it accepts JSON 5 data, and various other
 * options.
 * <p>
 * {@link Json} instances can be obtained via a builder: {@link #jsonBuilder()},
 * {@link #json5Builder()}, but ready-to-use presets can also be obtained via
 * {@link #json()}, {@link #json5()}, {@link #compactJson()} and
 * {@link #compactJson5()} (the latter two will print compact JSON, while the
 * other two print pretty JSON).
 */
public class Json {
	...
	/**
	 * Returns a builder with the standard JSON as defaults, with pretty
	 * formatting.
	 *
	...
	/**
	 * Returns a standard JSON preset with {@linkplain ParsingConfig#standard()
	 * standard parsing} and {@linkplain FormattingConfig#pretty() pretty
	 * formatting}.
	 *
	...
	/**
	 * Returns a JSON 5 preset with {@linkplain ParsingConfig#standard()
	 * standard parsing} and {@linkplain FormattingConfig#pretty() pretty
	 * formatting}.
	 *
	...
	/**
	 * Returns a standard JSON preset with {@linkplain ParsingConfig#standard()
	 * standard parsing} and {@linkplain FormattingConfig#compact() compact
	 * formatting}.
	 *
	 * @return A standard JSON preset
	 */
	...
	/**
	 * Parses a {@link Reader} as JSON. The reader will <strong>not</strong> be
	 * closed.
	 *
	...
	/**
	 * Parses an {@link InputStream} as JSON, using the
	 * {@linkplain Charset#defaultCharset() default charset}. The stream will
	 ...
	 * @throws JsonSyntaxException   When the JSON has invalid syntax
	 * @throws FileNotFoundException When the file cannot be found, is a directory, or could not
	 *                               be opened for some other reason
	 * @throws IOException           If an I/O error occurs
	 */
	...
	/**
	 * Serializes JSON to a {@link Writer}. The writer will <strong>not</strong>
	 * be closed.
	 *
	 * @param writer The writer to serialize to
	 * @throws NullPointerException   If any parameter is null
	 * @throws IncorrectTypeException If the given node is not an array or object and the
	 *                                formatting config does not allow any value
	 * @throws IOException            If an I/O error occurs
	 */
	...
	/**
	 * Serializes JSON to an {@link OutputStream}, using the
	 * {@linkplain Charset#defaultCharset() default charset}. The stream will
	 * <strong>not</strong> be closed.
	 *
	...
	public static class Builder {
		...
		/**
		 * Sets a custom {@link FormattingConfig}. The configuration is copied
		 * upon {@link #build()}. Modification of the configuration while
		 * building will affect the future {@link Json} instance
		 *
		...
		/**
		 * Sets a custom {@link ParsingConfig}. The configuration is copied upon
		 * {@link #build()}. Modification of the configuration while building
		 * will affect the future {@link Json} instance
		 *
		 * @param config The configuration
		 * @return This instance for chaining
		 * @throws NullPointerException When the configuration is null
