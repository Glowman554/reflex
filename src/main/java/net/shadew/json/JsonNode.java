...
/**
 * A general interface representing a JSON tree. This interface covers any type
 * a JSON node can be: a string, number, boolean, null, array or object. The
 * implementation is provided internally and this interface must not be manually
 * implemented.
 * <p>
 * A not-construct node (null, a number, a string or a boolean) is immutable and
 * any attempt to mutation of this node will throw an exception for them.
 * Copying primitives is essentially useless.
 * <ul>
 ...
 * </ul>
 * <p>
 * An array node is mutable and can be modified using array modification
 * methods. Using object modification methods will cause an exception, and so
 * does trying to obtain the primitive value of an array. Some methods will work
 ...
 * </ul>
 * <p>
 * An object node is mutable and can be modified using object modification
 * methods. Using array modification methods will cause an exception, and so
 * does trying to obtain the primitive value of an object. Some methods will
 * work on both arrays and objects.
 * <ul>
 ...
 * </ul>
 * <p>
 * Java's default methods, {@link #equals}, {@link #hashCode} and
 * {@link #toString} work on {@link JsonNode} instances. Converting a node to a
 * string will generate a quick and compact JSON representation of the node.
 * This is useful for debugging purposes, but it's not recommended when writing
...
public interface JsonNode extends Iterable<JsonNode>, JsonRepresentable {
	...
	/**
	 * The only {@link JsonNode} representing the JSON boolean value
	 * {@code true}.
	 */
	...
	/**
	 * Returns {@code node}, or the {@link #NULL} instance if {@code node} is
	 * null (Java null). This ensures the returned value is never null. This
	 * also checks whether the given {@link JsonNode} is a correct
	 * implementation, and not an external implementation.
	 *
	 * @param node A nullable JSON tree.
	 * @return The given JSON tree, or {@link #NULL}.
	 * @throws IllegalArgumentException When the given {@link JsonNode} implementation is not the
	...
	/**
	 * Returns a boolean-type node with the given boolean value. This returns
	 * {@link #TRUE} if the given value is true, and {@link #FALSE} if the given
	 * value is false. This returns {@link #NULL} if the given value is null.
	 *
	...
	/**
	 * Returns a string-type node with the given string value,
	 * {@link #EMPTY_STRING} if the given string is empty, or {@link #NULL} if
	...
	/**
	 * Returns a number-type node with the given number value, {@link #ZERO} if
	 * the given number is zero, or {@link #NULL} if the given number is null.
	 * Even though any valid {@link Number} subclass is valid, it is recommended
	 * to use only Java's builtin number types (primitives, {@link BigInteger},
	 * {@link BigDecimal}, etc.). Other number types may cause unexpected or
	...
	/**
	 * Returns a new mutable array node, initially filled with the given
	 * elements. Any null element is automatically converted to {@link #NULL}.
	 *
	...
	/**
	 * Returns a new mutable array node, initially filled with the given
	 * strings. Any null element is automatically converted to {@link #NULL}.
	 *
	 * @param elems The elements in the array
	 * @return The JSON array node
	 */
	...
	/**
	 * Returns a new mutable array node, initially filled with the given
	 * numbers. Any null element is automatically converted to {@link #NULL}.
	 *
	 * @param elems The elements in the array
	 * @return The JSON array node
	 */
	...
	/**
	 * Returns a new mutable array node, initially filled with the given
	 * booleans. Any null element is automatically converted to {@link #NULL}.
	 *
	 * @param elems The elements in the array
	 * @return The JSON array node
	 */
	...
	/**
	 * Returns a new mutable array node, initially filled with the given number.
	 * Any null element is automatically converted to {@link #NULL}.
	 *
	 * @param elems The elements in the array
	 * @return The JSON array node
	 */
	...
	/**
	 * Returns a new mutable array node, initially filled with the given amount
	 * of null nodes.
	 *
	...
	/**
	 * Returns a new mutable object node, initially filled with the elements
	 * from the given map. Elements are inserted in the given map's iteration
	 * order (which is preserved). Any null element is automatically replaced
	 * with {@link #NULL}.
	 *
	...
	/**
	 * Returns a new mutable object node, initially filled with the strings from
	 * the given map. Elements are inserted in the given map's iteration order
	 * (which is preserved). Any null element is automatically replaced with
	 * {@link #NULL}.
	 *
	...
	/**
	 * Returns a new mutable object node, initially filled with the numbers from
	 * the given map. Elements are inserted in the given map's iteration order
	 * (which is preserved). Any null element is automatically replaced with
	 * {@link #NULL}.
	 *
	...
	/**
	 * Returns a new mutable object node, initially filled with the booleans
	 * from the given map. Elements are inserted in the given map's iteration
	 * order (which is preserved). Any null element is automatically replaced
	 * with {@link #NULL}.
	 *
	...
	/**
	 * Attempts to convert a Java-based structure (i.e. Maps, Lists, etc.) to a
	 * JSON tree.
	 * <p>
	 * <strong>Warning:</strong> Even though this method allows to obtain a JSON
	 * tree quickly from pretty much any basic builtin type, this method is not
	 * recommended for production use. This method does some implicit
	 * conversions to primitive types in order to create a suitable JSON
	 * structure, which can happen in semi-unpredictable ways. One might prefer
	 * more control over how instances are converted to JSON. Use with a light
	 * risk. For a more flexible way to convert to JSON trees, use
	 * {@link JsonCodec}s.
	 * <p>
	 ...
	 * <li>Any {@link JsonRepresentable} is converted via
	 * {@link JsonRepresentable#toJson()}, implement this interface to allow
	 * quick serialization</li>
	 ...
	 * <li>Any {@link File}, {@link Path}, {@link URL}, {@link URI} or
	 * {@link JsonPath} is converted into a string via
	 * {@link Object#toString()}</li>
	 * <li>Any {@link Enum} is converted into its name via
	 * {@link Enum#name()}</li>
	 * <li>Any {@link Stream} is converted to an array node with the streamed
	 * elements</li>
	 * <li>Any {@link Iterable} is converted to an array node with the iterated
	 * elements</li>
	 * <li>Any {@link Iterator} is converted to an array node with the remaining
	 * elements</li>
	 * <li>Any {@link Enumeration} is converted to an array node with the
	 * remaining elements</li>
	 * <li>Any {@link Map} is converted to an object node with the contained
	 * elements, keys converted to string as by {@link #toString()}</li>
	 * <li>Any {@link Dictionary} is converted to an object node with the
	 * contained elements, keys converted to string as by
	 * {@link #toString()}</li>
	 * <li>Any array type (primitive or not) is converted to an array node with
	 * the contained elements</li>
	 * <li>Any other object is converted to a string like
	 * {@link Object#toString()}</li>
	 * </ul>
	 * Conversions of objects in iterables/arrays/maps/etc. are recursively
	 * converted using this method as well.
	 *
	 * @param obj The object to convert.
	 * @return The converted JSON tree
	 * @deprecated This method makes lots of assumptions and is only for debug
	 * purposes. For production purposes, use a {@link JsonCodec}.
	 */
	@Deprecated
	static JsonNode fromJavaObject(Object obj) {
		if (obj == null) {
			...
			// Serialize anything specifically serializable in the way
			// it wants to
	...
	/**
	 * Returns a {@link Collector} that collects {@link JsonNode}s into an
	 * {@linkplain JsonType#ARRAY array} node.
	 *
	...
	/**
	 * Returns true when this node is a {@linkplain JsonType#STRING string}
	 * node.
	 *
	...
	/**
	 * Returns true when this node is a {@linkplain JsonType#NUMBER number}
	 * node.
	 *
	...
	/**
	 * Returns true when this node is a {@linkplain JsonType#BOOLEAN boolean}
	 * node.
	 *
	...
	/**
	 * Returns true when this node is an {@linkplain JsonType#OBJECT object}
	 * node.
	 *
	...
	/**
	 * Returns true when this node is a {@linkplain JsonType#isPrimitive()
	 * primitive} node.
	 *
	...
	/**
	 * Returns true when this node is a {@linkplain JsonType#isConstruct()
	 * construct} node.
	 *
	...
	/**
	 * Throws an {@link IncorrectTypeException} when this node is not
	...
	/**
	 * Throws an {@link IncorrectTypeException} when this node is
	 * {@linkplain JsonType#NULL null}
	 *
	 * @return This instance for chaining
	...
	/**
	 * Throws an {@link IncorrectTypeException} when this node is not the given
	 * type
	 *
	 * @return This instance for chaining
	...
	/**
	 * Throws an {@link IncorrectTypeException} when this node is not one of the
	 * given types
	 *
	 * @return This instance for chaining
	...
	/**
	 * Throws an {@link IncorrectTypeException} when this node is one of the
	 * given types
	 *
	 * @return This instance for chaining
	...
	/**
	 * Performs the given action if this node is a {@linkplain JsonType#STRING
	 * string}. The consumer must take two arguments: this {@link JsonNode}, and
	...
	/**
	 * Performs the given action if this node is a {@linkplain JsonType#NUMBER
	 * number}. The consumer must take two arguments: this {@link JsonNode}, and
	 ...
	 * Performs the given action if this node is a {@linkplain JsonType#NUMBER
	 * number}. The consumer must take two arguments: this {@link JsonNode}, and
	 * the {@link BigInteger} value of this node.
	 *
	 * @param action The action to perform when the check passes
	 * @return This instance for chaining
	 * @throws NullPointerException If the action is null
	 ...
	 * Performs the given action if this node is a {@linkplain JsonType#NUMBER
	 * number}. The consumer must take two arguments: this {@link JsonNode}, and
	 * the {@link BigDecimal} value of this node.
	 *
	 * @param action The action to perform when the check passes
	 * @return This instance for chaining
	 * @throws NullPointerException If the action is null
	...
	/**
	 * Performs the given action if this node is a {@linkplain JsonType#BOOLEAN
	 * boolean}. The consumer must take two arguments: this {@link JsonNode},
	 * and the boolean value of this node.
	 *
	 * @param action The action to perform when the check passes
	 * @return This instance for chaining
	 * @throws NullPointerException If the action is null
	...
	/**
	 * Performs the given action if this node is {@linkplain JsonType#NULL null}
	 * (JSON null). The consumer must take this {@link JsonNode} as argument.
	 *
	 * @param action The action to perform when the check passes
	 * @return This instance for chaining
	 * @throws NullPointerException If the action is null
	...
	/**
	 * Performs the given action if this node is an {@linkplain JsonType#ARRAY
	 * array}. The consumer must take this {@link JsonNode} as argument.
	 *
	 * @param action The action to perform when the check passes
	 * @return This instance for chaining
	 * @throws NullPointerException If the action is null
	...
	/**
	 * Performs the given action if this node is an {@linkplain JsonType#OBJECT
	 * object}. The consumer must take this {@link JsonNode} as argument.
	 *
	 * @param action The action to perform when the check passes
	 * @return This instance for chaining
	 * @throws NullPointerException If the action is null
	...
	/**
	 * Performs the given action if this node is a
	 * {@linkplain JsonType#isPrimitive() primitive}. The consumer must take
	...
	/**
	 * Performs the given action if this node is a
	 * {@linkplain JsonType#isConstruct() construct}. The consumer must take
	 * this {@link JsonNode} as argument.
	 *
	 * @param action The action to perform when the check passes
	 * @return This instance for chaining
	...
	/**
	 * Returns the string value of this node. Unlike {@link #asExactString()},
	 * this will turn any not-construct into a string without throwing an
	 * exception.
	 *
	 * @return The string value of this node
	...
	/**
	 * Returns a {@link Number} value of this node, in of any type that fits
	 * best.
	 *
	 ...
	 * @throws IncorrectTypeException When this node is not an array node or when one of the
	 *                                array's elements is not a string node
	 */
	 ...
	 * @throws IncorrectTypeException When this node is not an array node or when one of the
	 *                                array's elements is not a number node
	 */
	byte[] asByteArray();
	...
	/**
	 * Returns a {@link Number} array value of this node, numbers being in of
	 * any type that fits best.
	 *
	 ...
	 * @throws IncorrectTypeException When this node is not an array node or when one of the
	 *                                array's elements is not a boolean node
	 */
	boolean[] asBooleanArray();

	/**
	 * Returns the string array value of this node, enforcing the specified
	 * length. An exception is thrown if any other length of array is present.
	 *
	 * @param fixedLength The required length of the array
	 * @return The string array value of this node
	 * @throws IncorrectTypeException When this node is not an array node or when one of the
	 *                                array's elements is not a string node
	 * @throws IncorrectSizeException When the actual length of this array node is not the required
	 *                                length
	 */
	String[] asStringArray(int fixedLength);

	/**
	 * Returns the byte array value of this node, enforcing the specified
	 * length. An exception is thrown if any other length of array is present.
	 *
	 * @param fixedLength The required length of the array
	 * @return The byte array value of this node
	...
	/**
	 * Returns the short array value of this node, enforcing the specified
	 * length. An exception is thrown if any other length of array is present.
	 *
	 * @param fixedLength The required length of the array
	 * @return The short array value of this node
	...
	/**
	 * Returns the int array value of this node, enforcing the specified length.
	 * An exception is thrown if any other length of array is present.
	 *
	 * @param fixedLength The required length of the array
	 * @return The int array value of this node
	 * @throws IncorrectTypeException When this node is not an array node or when one of the
	...
	/**
	 * Returns the long array value of this node, enforcing the specified
	 * length. An exception is thrown if any other length of array is present.
	 *
	 * @param fixedLength The required length of the array
	 * @return The long array value of this node
	...
	/**
	 * Returns the float array value of this node, enforcing the specified
	 * length. An exception is thrown if any other length of array is present.
	 *
	 * @param fixedLength The required length of the array
	 * @return The float array value of this node
	...
	/**
	 * Returns the double array value of this node, enforcing the specified
	 * length. An exception is thrown if any other length of array is present.
	 *
	 * @param fixedLength The required length of the array
	 * @return The double array value of this node
	...
	/**
	 * Returns the {@link BigInteger} array value of this node, enforcing the
	 * specified length. An exception is thrown if any other length of array is
	 * present.
	 *
	 * @param fixedLength The required length of the array
	 * @return The {@link BigInteger} array value of this node
	...
	/**
	 * Returns the {@link BigDecimal} array value of this node, enforcing the
	 * specified length. An exception is thrown if any other length of array is
	 * present.
	 *
	 * @param fixedLength The required length of the array
	...
	/**
	 * Returns the {@link Number} array value of this node, enforcing the
	 * specified length. An exception is thrown if any other length of array is
	 * present.
	 *
	 * @param fixedLength The required length of the array
	...
	/**
	 * Returns the boolean array value of this node, enforcing the specified
	 * length. An exception is thrown if any other length of array is present.
	 *
	 * @param fixedLength The required length of the array
	 * @return The boolean array value of this node
	...
	/**
	 * Returns a list containing the elements of this array node. The list is a
	 * direct copy of the array and can be modified without modifying the array
	 * node itself. It does not reflect this node.
	 *
	...
	/**
	 * Returns a map containing the elements of this object node, mapped to
	 * their keys. The map is a direct copy of the object and can be modified
	 * without modifying the object node itself. It does not reflect this node.
	 *
	...
	/**
	 * Returns the element of this array at the given index. Negative indices
	 * index from the end.
	 *
	...
	/**
	 * Sets the element of this array at the given index. Negative indices index
	 * from the end. A null value is converted to {@link #NULL}.
	 *
	 * @param index The index
	 ...
	 * @throws IndexOutOfBoundsException If the index falls out of the bounds of this array
	 * @throws IncorrectTypeException    If this node is not an array
	 * @throws IllegalArgumentException  When the given {@link JsonNode} implementation is not built
	 *                                   in
	 */
	JsonNode set(int index, JsonNode value);

	/**
	 * Sets the element of this array at the given index to a string. Negative
	 * indices index from the end. A null value is converted to {@link #NULL}.
	 *
	 * @param index The index
	 * @param value The new value
	 * @return This instance for chaining
	...
	/**
	 * Sets the element of this array at the given index to a number. Negative
	 * indices index from the end. A null value is converted to {@link #NULL}.
	 *
	 * @param index The index
	 * @param value The new value
	...
	/**
	 * Sets the element of this array at the given index to a boolean. Negative
	 * indices index from the end. A null value is converted to {@link #NULL}.
	 *
	 * @param index The index
	 * @param value The new value
	...
	/**
	 * Adds a new element to the end of this array. A null value is converted to
	 * {@link #NULL}.
	 *
	 * @param value The new value
	 * @return This instance for chaining
	...
	/**
	 * Adds a new string element to the end of this array. A null value is
	...
	/**
	 * Adds a new number to the end of this array. A null value is converted to
	 * {@link #NULL}.
	 *
	 * @param value The new value
	 * @return This instance for chaining
	...
	/**
	 * Adds a new boolean to the end of this array. A null value is converted to
	 * {@link #NULL}.
	 *
	 * @param value The new value
	 * @return This instance for chaining
	...
	/**
	 * Inserts a new element at a certain position in this array, before the
	 * element at the given index. Negative indices index from the end (note
	 * that index -1 does <strong>not</strong> add it to the end, but before the
	 * last element). A null value is converted to {@link #NULL}.
	 *
	...
	/**
	 * Inserts a new string element at a certain position in this array, before
	 * the element at the given index. Negative indices index from the end (note
	 * that index -1 does <strong>not</strong> add it to the end, but before the
	 * last element). A null value is converted to {@link #NULL}.
	 *
	 * @param index The index to insert at
	...
	/**
	 * Inserts a new number element at a certain position in this array, before
	 * the element at the given index. Negative indices index from the end (note
	 * that index -1 does <strong>not</strong> add it to the end, but before the
	 * last element). A null value is converted to {@link #NULL}.
	 *
	...
	/**
	 * Inserts a new boolean element at a certain position in this array, before
	 * the element at the given index. Negative indices index from the end (note
	 * that index -1 does <strong>not</strong> add it to the end, but before the
	 * last element). A null value is converted to {@link #NULL}.
	 *
	...
	/**
	 * Removes the element at the given index in this array. Negative indices
	 * index from the end.
	 *
	...
	/**
	 * Returns the size of this array or object, i.e. the amount of elements in
	 * this container.
	 *
	...
	/**
	 * Returns the size of this array or object if it's an array or object, or
	 * the length of this string if it's a string.
	 *
	...
	/**
	 * Enforces this array or object to have a specific number of elements,
	 * throwing an {@link IncorrectSizeException} when it is not the correct
	 * size.
	 *
	...
	/**
	 * Appends all the elements from the given array to this array. It does not
	...
	/**
	 * Prepends all the elements from the given array to this array. It does not
	 * modify the given array.
	 *
	 * @return This instance for chaining
	 * @throws IncorrectTypeException If this node, or the supplied node, is not an array
	...
	/**
	 * Slices this array, removing all elements except those in the given range.
	 * This operation modifies this array. To obtain a sliced copy make a copy
	 * first: {@code array.copy().slice(...)}.
	 *
	 * @param from The start index, negative values count from the end
	 * @param to   The end index, must be more than start
	 * @return This instance for chaining
	 * @throws IncorrectTypeException    If this node, or the supplied node, is not an array
	 * @throws IndexOutOfBoundsException If one of the given indices is more than the length of this
	 *                                   array, or less than negative the length of this array.
	...
	/**
	 * Returns the element at the specified key in this object. This returns
	 * null (not {@link #NULL}) when no value has been associated to this key. A
	...
	/**
	 * Replaces or adds a new element at the specified key in this object. A
	 * null value is converted to {@link #NULL}. A null key is treated as a
	 * literal key "null".
	 *
	...
	/**
	 * Replaces or adds a new string element at the specified key in this
	 * object. A null value is converted to {@link #NULL}. A null key is treated
	 * as a literal key "null".
	 *
	 * @param key   The key to set at
	 * @param value The new value
	 * @return This instance for chaining
	...
	/**
	 * Replaces or adds a new number element at the specified key in this
	 * object. A null value is converted to {@link #NULL}. A null key is treated
	 * as a literal key "null".
	 *
	 * @param key   The key to set at
	...
	/**
	 * Replaces or adds a new boolean element at the specified key in this
	 * object. A null value is converted to {@link #NULL}. A null key is treated
	 * as a literal key "null".
	 *
	 * @param key   The key to set at
	...
	/**
	 * Removes the element at the specified key in this object, if it was
	 * assigned. If no element was assigned, this method does nothing. A null
	 * key is treated as a literal key "null".
	 *
	...
	/**
	 * Returns whether a key is present in this object. A null key is treated as
	 * a literal key "null".
	 *
	...
	/**
	 * Returns whether a value is present in this object or array. A null value
	 * is treated as a JSON null.
	 *
	...
	/**
	 * Returns a set of all the keys in this object node. This set reflects this
	 * node, and removing from this set means removing from this node.
	 *
	...
	/**
	 * Returns a collection of all the values in this object or array node. This
	 * collection reflects this node, and removing from this set means removing
	 * from this node.
	 *
	...
	/**
	 * Returns a set of all the key-value pairs in this object node. This set
	 * reflects this node, and removing from this set means removing from this
	 * node.
	 *
	...
	/**
	 * Returns a stream of all the values in this object or array node. This
	 * stream is ordered and not parallel. In case of arrays, the stream order
	 * will be the array order. In case of objects, the stream order will be the
	 * insertion order (which is the order of entries in a parsed document as
	 * well).
	 *
	...
	/**
	 * Loops over all key-value pairs in this object, passing them into the
	...
	/**
	 * Queries this JSON tree with the given path string, see
	 * {@link JsonPath#parse(String)} for the syntax of this path. This method
	 * will return null (not {@link #NULL}) when the last query was on an object
	 * that did not contain the queried key.
	 *
	 * @param path The path to query by
	 * @return The node that was navigated to, or null
	 * @throws IncorrectTypeException    When trying to query a key from a non-object or an index from
	 *                                   a non-array.
	 * @throws IndexOutOfBoundsException When querying an array at an out-of-range index
	 * @throws NoSuchElementException    When trying to query on a nonexisting value (i.e. {@code a.b}
	 *                                   would try to query {@code b} from {@code a} while {@code a}
	...
	/**
	 * Queries this JSON tree with the given path. This method will return null
	 * (not {@link #NULL}) when the last query was on an object that did not
	...
	/**
	 * Returns a deep copy of this tree. If this is a primitive (string, number,
	 * boolean or null), this will return itself (since these instances are
	 * immutable). If this is an array, it will return a new array with a deep
	 * copy of each element in the same order. If this is an object, it will
	 * return a new object with a deep copy of each element, under their
	 * respective keys and in the same order.
	 *
	...
	/**
	 * Returns a shallow copy of this tree. If this is a primitive (string,
	 * number, boolean or null), this will return itself (since these instances
	 * are immutable). If this is an array, it will return a new array with the
	 * same nodes in the same order. If this is an object, it will return a new
	 * object with the same nodes under their respective keys, and in the same
	 * order.
	 *
	...
	/**
	 * Implementation of {@link JsonRepresentable}. This method will return this
	 * node, as it is already a {@link JsonNode}.
	 *
	 * @return This node.
	 */
	@Override
