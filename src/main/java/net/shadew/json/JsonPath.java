...
/**
 * A pre-parsed JSON path. A path can be parsed from a JavaScript-like syntax.
 * See {@link #parse(String)} for exact syntax rules.
 * <p>
 * A {@link JsonPath} instance is immutable and any modifying method will return
 * a modified copy of the original.
 */
public final class JsonPath {
	/**
	 * The root path, a.k.a. the path that references the root element in a JSON
	 * tree. The root path is the path that serializes to an empty string, and
	 * is the path returned by {@link #parse(String)} when a blank string is
	 * entered.
	 * <p>
	...
	/**
	 * Returns a path to element {@code index} of the root array, which is
	 * essentially the path {@code [index]}. This is the same as {@link #ROOT
	 * JsonPath.ROOT}{@code .}{@link #index(int) index}{@code (index)}.
	 *
	...
	/**
	 * Returns a path to element {@code key} of the root object, which is
	 * essentially the path {@code [key]}. This is the same as {@link #ROOT
	 * JsonPath.ROOT}{@code .}{@link #member(String) member}{@code (key)}.
	 *
	...
	/**
	 * Parses a serialized JSON path. This accepts a similar syntax as
	 * JavaScript. A path consists of zero or more entries, which are either
	 * identified by a period with an identifier ({@code .entry}) or a string or
	 * number between brackets ({@code [3]}, {@code ["entry"]}). The first entry
	 * may also be an identifier without period ({@code entry}).
	 * <p>
	 * Formal syntax rules:
	 * <ul>
	 * <li><em>Path</em> := <strong>Nothing</strong> | <em>FirstPathEntry</em>
	 * <em>PathEntry*</em></li>
	 * <li><em>FirstPathEntry</em> := <em>Identifier</em> |
	 * <em>PathEntry</em></li>
	 * <li><em>PathEntry</em> := <code>'.'</code> <em>Identifier</em> |
	 * <em>IndexEntry</em> | <em>StringEntry</em></li>
	 * <li><em>IndexEntry</em> := <code>'['</code> <em>Number</em>
	 * <code>']'</code></li>
	 * <li><em>StringEntry</em> := <code>'['</code> <em>String</em>
	 * <code>']'</code></li>
	 * <li><em>Identifier</em> := <em>IdentifierStart</em>
	 * <em>IdentifierChar</em>*</li>
	 * <li><em>String</em> := <code>'"'</code> <em>CharacterOrSingleQuote</em>*
	 * <code>'"'</code></li>
	 * <li><em>DoubleQuoteString</em> := <code>'"'</code>
	 * <em>DoubleQuoteChar</em>* <code>'"'</code></li>
	 * <li><em>SingleQuoteString</em> := <code>"'"</code>
	 * <em>SingleQuoteChar</em>* <code>"'"</code></li>
	 * <li><em>Number</em> := <em>Sign</em>? <em>Digit</em>+</li>
	 * <li><em>Digit</em> := <code>[0-9]</code></li>
	 * <li><em>Sign</em> := <code>'+'</code> | <code>'-'</code></li>
	 * <li><em>IdentifierStart</em> := <strong>Unicode Identifier Start</strong>
	 * | <em>UnicodeEscape</em></li>
	 * <li><em>IdentifierChar</em> := <strong>Unicode Identifier Part</strong> |
	 * <em>UnicodeEscape</em></li>
	 * <li><em>DoubleQuoteChar</em> := <em>Character</em> |
	 * <code>"'"</code></li>
	 * <li><em>SingleQuoteChar</em> := <em>Character</em> |
	 * <code>'"'</code></li>
	 * <li><em>Character</em> := <strong>Any Character</strong> |
	 * <em>Escape</em></li>
	 * <li><em>Escape</em> := <em>UnicodeEscape</em> | <code>'&#92;'</code>
	 * <strong>Any Character</strong></li>
	 * <li><em>UnicodeEscape</em> := <code>'&#92;u'</code> <em>HexDigit</em>
	 * <em>HexDigit</em> <em>HexDigit</em> <em>HexDigit</em></li>
	 * <li><em>HexDigit</em> := <code>[0-9a-fA-F]</code></li>
	 * </ul>
	 * Here, <strong>Unicode Identifier Start</strong> is any character matched
	 * by {@link Character#isUnicodeIdentifierStart(char)}, <strong>Unicode
	 * Identifier Part</strong> is any character matched by
	 * {@link Character#isUnicodeIdentifierPart(char)}, and <strong>Any
	 * Character</strong> is, obviously, any character.
	 * <p>
	 * A special case is where the path is an empty or blank string. In that
	 * case this method will return {@link #ROOT}.
	 *
	 * @param path The path to parse.
	 * @return The parsed path.
	 * @throws IllegalArgumentException If the given path string does not match the above syntax
	 *                                  rules.
	...
	public static JsonPath parse(String path) {
		...
		// If it still happens, parser implementation is incorrect or the quick
		// check did not work properly
		if (steps.size() == 0) {
	...
	// Reads a full path step (the 'first' argument indicate whether identifiers
	// may start without period)
	...
	private static Step readId(Reader reader) {
		...
		if (c == '.') // Ignore period if present, this is already enforced in
			// readStep
			reader.skip();
		...
		// TODO It is essentially possible to allow any identifier character as
		// first character, decide if that's wanted
		// Advantage: it allows some more strings to be identifiers
		// Disadvantage: numeric identifiers (i.e. path.3.y is possible) can be
	...
	/**
	 * Returns the path to the parent of the object or array referenced by this
	 * path. This essentially returns the same path as this path, but without
	 * the last entry.
	 * <ul>
	...
	/**
	 * Returns the path to element <code>index</code> of the array referenced by
	 * this path. This is similar to calling {@link #resolve(String)
	 * resolve}{@code ("[" + index + "]")}.
	 *
	 * @param index The index, zero based. When negative, it indexes arrays from
	 *              the end.
	...
	/**
	 * Returns the path to member <code>key</code> of the object referenced by
	 * this path. This is similar to calling {@link #resolve(String)
	 * resolve}{@code ("['" + key + "']")}, although characters in the key must
	 * be properly escaped.
	 *
	 * @param key The key to access. When null, the literal key "null" is
	 *            accessed.
	 * @return The child path.
	 */
	...
	/**
	 * Resolves the given path {@code p} against this path. For example, if this
	 * path is {@code a.b}, and {@code p} is {@code p[1]}, then the returned
	 * path is {@code a.b.p[1]}.
	 *
	...
	/**
	 * Parses and resolves the given path {@code p} against this path. For
	 * example, if this path is {@code a.b}, and {@code p} is {@code p[1]}, then
	 * the returned path is {@code a.b.p[1]}. This is the same as
	 * {@link #resolve(JsonPath) resolve}{@code (}{@link #parse
	 * JsonPath.parse}{@code (p))}.
	 *
	 * @param p The subpath to resolve.
	 * @return The fully resolved path.
	 * @throws NullPointerException If the given path {@code p} is null.
	...
	/**
	 * Resolves this path against the given path {@code p}. For example, if this
	 * path is {@code a.b}, and {@code p} is {@code p[1]}, then the returned
	 * path is {@code p[1].a.b}.
	 *
	...
	/**
	 * Parses and resolves this path against the given path {@code p}. For
	 * example, if this path is {@code a.b}, and {@code p} is {@code p[1]}, then
	 * the returned path is {@code p[1].a.b}. This is the same as
	 * {@link #resolveTo(JsonPath) resolveTo}{@code (}{@link #parse
	 * JsonPath.parse}{@code (p))}.
	 *
	 * @param p The parent path to resolve to.
	 * @return The fully resolved path.
	...
	/**
	 * Returns a string representation of this path. The string representation
	 * of a path is a JavaScript-like path string. This representation can be
	 * parsed using {@link #parse(String)} to obtain a path that equals this
	 * path.
	 * <p>
	 * This representation is unique for each different path, meaning that,
	 * although it's not as efficient, {@link JsonPath}s can be compared like
	 * {@link JsonPath#equals} by comparing their string representations via
	 * {@link String#equals}.
	 */
	@Override
	...
	private static class Reader {
		...
		IllegalArgumentException positionalError(String error) {
			return new IllegalArgumentException( // TODO Different exception
					// type?
					String.format("Invalid pattern: %s (at pos: %d)%n", error, pos) + string + System.lineSeparator() + " ".repeat(pos) + "^");
		}
