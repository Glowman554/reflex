	...
	private enum JsonLexerState implements LexerState {
		...
		IDENTIFIER_ESCAPE {
			...
			public Token lex(int c, AbstractLexer lex) throws JsonSyntaxException {
				...
				lex.remember(2); // Special sign for unicode escape states that
				// we're in an identifier
				lex.store('\0');
				lex.state(UNICODE_ESCAPE_1);
				return null;
			}
		...
		START_IDENTIFIER_ESCAPE {
			...
			public Token lex(int c, AbstractLexer lex) throws JsonSyntaxException {
				...
				lex.remember(1); // Special sign for unicode escape states that
				// we're in an identifier start
				lex.store('\0');
				lex.state(UNICODE_ESCAPE_1);
				return null;
			}
