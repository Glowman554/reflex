...
/**
 * A codec that encodes or decodes a record. This codec class automatically
 * manages the checks and exceptions thrown by inner codecs, and ensures
 * required fields are present.
 * <p>
 * In terms of JSON codecs, a record is a flat JSON object with some properties.
 * Flat means that all fields of the encoded object have a separate property in
 * the encoded JSON, and are not nested into inner structures if they are
 * neither in the Java object. For example, a person object with a first name
 * and a last name as separate fields will become {@code {"firstName": ...,
 * "lastName": ...}}, but a person object with one name field of a name record
 * will encode like {@code {"name": {"first": ..., "last": ...}}}.
 * <p>
 * A {@link RecordCodec} will always require a JSON object, not an array or
 * primitive. It will not encode null values.
 */
public abstract class RecordCodec<A> implements JsonCodec<A> {
	private final ThreadLocal<ContextStack<DecodeContextImpl<A>>> decodeStack = ThreadLocal.withInitial(() -> new ContextStack<>(DecodeContextImpl::new));

	private final ThreadLocal<ContextStack<EncodeContextImpl<A>>> encodeStack = ThreadLocal.withInitial(() -> new ContextStack<>(EncodeContextImpl::new));
	 ...
	 * Decodes the record.
	 *
	 * @param ctx The decode context, which wraps the JSON tree and checks the
	 *            presence of required fields when reading them. This context is
	 *            recycled after decoding, do not try to cache it.
	 ...
	 * Encodes the record.
	 *
	 * @param ctx The encode context, which wraps the JSON tree and ensures
	 *            fields are encoded the right way. This context is recycled
	 *            after encoding, do not try to cache it.
	...
	public interface DecodeContext<I> {
		...
		/**
		 * Get the value of a required field, failing if the field is not
		 * present.
		 *
		...
		/**
		 * Get the value of an optional field, returning null if the field is
		 * not present.
		 *
		 * @param key The name of the field
		...
		/**
		 * Get the value of an optional field, passing it on to the given
		 * function if the field is present.
		 *
		 * @param key     The name of the field
		 ...
		 * @param key   The name of the field
		 * @param codec The codec of the field's type
		 * @return The field value, decoded by the given codec, or null if
		 * absent
		 */
